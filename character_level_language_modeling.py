...
def reshape_data(sequence, batch_size, num_steps):
    ...
    ##truncate the sequence at the end to get rid of remaining characters that do not make a full batch
    x = sequence[0:num_batches * tot_batch_length]
    y = sequence[1: num_batches * tot_batch_length + 1]
    ##split x and y into a list batches of sequence
...
class CharRNN(object):
    ...
    def build(self, sampling):
        ...
        # build the multilayer RNN cells
        cells = tf.nn.rnn_cell.MultiRNNCell(
            [tf.nn.rnn_cell.ResidualWrapper(tf.nn.rnn_cell.LSTMCell(self.lstm_size), tf_keepprob) for _ in
             range(self.num_layers)])

        # define hte initial state
    ...
    def train(self, train_x, train_y, num_epochs, ckpt_dir='./model/'):
        # create the checkpoint directory if it doesn't exists
